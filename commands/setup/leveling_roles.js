...
module.exports = {
  ...
  if (sub === "add") {
        ...
        let role = interaction.options.getRole("role") || interaction.guild.roles.cache.find(r => r.id === roles.join(" ") || r.name === roles.join(" ").toLowerCase())
        if(!level) {
            return interaction.editReply({content: `${bot.crosss} • Please supply a level!`})
        }
        if(!role){
            return interaction.editReply({content: `${bot.crosss} • Please supply a role!`})
        }
        ...
        return interaction.editReply({content: `${bot.tick} • Added! now at level \`${level}\` you will gain the **${role.name}** role!`})
}
...
if (sub === "remove") {
  ...
  if(guild.leveling_roles.length <= 0) {
            return interaction.editReply({content: `${bot.crosss} • Please add some level auto roles first!`})
        ...
        if(valid) {
            ...
            guild.save()
            return interaction.editReply({content: `${bot.tick} • Removed! now at level \`${level}\` you will not get **${role.name}** role!`})
        } else {
            return interaction.editReply({content: `${bot.crosss} • Please supply a valid leveling auto role!`})
        }
