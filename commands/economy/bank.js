...
module.exports = {
    ...
    run: async (bot, interaction, args) => {
      ...
      const currency = bot.eco.currency //bot.emoji("currencyy_Blossomii")
  ...
  await bot.eco.GetUser({UserID: user1 ? user1.id : user.id})
   ...
   const embed = new EmbedBuilder()
    .setAuthor({name:`Balance of ${user1 ? user1.tag : user.tag}`, iconURL: (user1 ? user1 : user).avatarURL({dynamic: true})})
    .setColor(bot.color)
