...
module.exports = {
  ...
  name: "rename-emoji",
  aliases: ["rename"],
  description: "Renames server emoji",
  ...
  if (!message.member.hasPermission("MANAGE_EMOJIS")) {
    ...
    if (!message.guild.me.hasPermission("MANAGE_EMOJIS")) {
      ...
      if (!args[0]) return message.channel.send("emote is a required argument that is missing.");if (!args[1]) 
      return message.channel.send("name is a required argument that is missing.")
      ...
      } else { return message.channel.send(":x: | Emoji not found")}
