const { CommandInteraction, MessageEmbed, MessageCollector, MessageSelectMenu, MessageActionRow, Util } = require("discord.js")
      ...
      await interaction.deleteReply()
        ...
        collector.on("collect", (msg) => {
            ...
            if(msg.content.toLowerCase() === "cancel") {
                ...
                msg.delete().catch(() => {})
                interaction.channel.send({content: `${bot.tick} • Process has been stopped!`})
            ...
            switch(counter) {
                ...
                case 2:
                    ...
                    if(msg.content.toLowerCase() === "default") {
                        ...
                        if(testHex(msg.content)) {
                            ...
                            interaction.channel.send({content: `${bot.crosss} • Please supply a valid hex code (E.G #FFFFFF or FFFFFF)`})
                ...
                case 4:
                    ...
                    if(msg.content.toLowerCase() === "bind") {
                        ...
                        channel = msg.mentions.channels.first() || interaction.guild.channels.cache.find(c => c.id === msg.content || c.name.toLowerCase() === msg.content.toLowerCase())
                    }
                    if(!channel) {
                        interaction.channel.send({content: `${bot.crosss} • Please supply a valid channel or use "bind" to bind it to the current channel!`})
                            collector.stop(stopreasons.ERR)
                            hoisterMessage.delete().catch(() => {})
                    }
                ...
                case 5:
                    if(msg.content.toLowerCase() === "done"){
                        if(finalData.roles.length <= 0) {
                            interaction.channel.send({content: `${bot.crosss} • Please supply some dropdown roles!`})
                            collector.stop(stopreasons.ERR)
                            hoisterMessage.delete().catch(() => {})
                            return;
                        }
                    ...
                    if(!parsedEmoji.id) {
                        interaction.channel.send({content: `${bot.crosss} • Please supply a valid emoji!`})
                        hoisterMessage.delete().catch(() => {})
                        collector.stop(stopreasons.ERR)
        ...
        collector.on("end", async (collected, reason) => {
            ...
            if(reason === stopreasons.FINSHED) {
                ...
                finalData.roles.forEach((r) => {
                    const serverRoles = interaction.guild.roles.cache.find(x => x.id === r.role)
                    const serverEmoji = interaction.guild.emojis.cache.find(e => e.id === r.viewEmoji)
...
if (sub === "list") {
       ...
       if(guild.dropdownRoles.length <= 0) {
            ...
            guild.dropdownRoles.forEach((dd) => {
                arr.push(` > **#${pos++})** \`${dd.id}\` • [\`Jump\`](https://discordapp.com/channels/${interaction.guild.id}/${dd.chanId}/${dd.msgId})  `)
            })
            const embed = new MessageEmbed()
