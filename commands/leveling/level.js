  ...
  category: "leveling",
	...
	if (user.bot) {
		const embed2 = new MessageEmbed()
			.setColor('#f04947')
			.setDescription(`${bot.config.wrongemoji} **| Sorry but we don't have levelling system for bots!**`);
	...
	if (!UserLvl) {
		const embed2 = new MessageEmbed()
			.setColor('#f04947')
			.setDescription(`${bot.config.wrongemoji} **| That user has not levelled up yet!**`);
		return message.channel.send(embed2);
	}
