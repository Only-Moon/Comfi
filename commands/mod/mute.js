	...
	run: async (bot, interaction, args) => {
		...
		if (sub === 'add') {
			...
			if (mutee === interaction.member)
				return interaction.editReply(
					`${bot.error} • **You Cannot Mute Yourself!**`
				)
			if (
				mutee.roles.highest.comparePositionTo(
			...
			if (mutee.user.bot)
				return interaction.editReply(`${bot.error} • **Cannot Mute Bots!**`)
			...
			let muteerole = interaction.guild.roles.cache.find(
				r => r.name === 'muted'
			)
			...
			if (mutee.roles.cache.has(muterole.id))
				return interaction.editReply(
					`${bot.error} • **User Is Already Muted!**`
				)
		...
		if (sub === 'remove') {
			try {
				...
				if (!muterole)
					return interaction.editReply(
						`${bot.error} • **There Is No Mute Role To Remove!**`
					)
				if (!mutee.roles.cache.has(muterole.id))
