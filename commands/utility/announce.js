...
const {
  EmbedBuilder, ActionRowBuilder, ButtonBuilder, ApplicationCommandOptionType, ButtonStyle, ChannelType,
} = require('discord.js');
...
module.exports = {
  ...
  run: async (bot, interaction, args) => {
    try {
      ...
      const everyonebtn = new ButtonBuilder().setCustomId('everyone').setLabel('@Everyone').setEmoji('📡')
        .setStyle(ButtonStyle.Success);
      const ghostbtn = new ButtonBuilder().setCustomId('ghostping').setLabel('Ghost-Ping').setEmoji('👻')
        .setStyle(ButtonStyle.Secondary);
      const herebtn = new ButtonBuilder().setCustomId('here').setLabel('@Here').setEmoji('🏠')
        .setStyle(ButtonStyle.Secondary);

      const delbtn = new ButtonBuilder().setCustomId('delete').setLabel('Delete').setEmoji('❌')
      ...
      const collector = await sent.createMessageComponentCollector({ time: 60000 });
