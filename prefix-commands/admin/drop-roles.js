...
module.exports = {
	config: {
		name: 'role-setup',
		aliases: ['set-roles', 'button-role', 'dp'],
		category: 'admin',
		description: 'Adds or Removes the selected role in autorole database',
		usage: 'autorole'
	},
	run: async (bot, message, args) => {
if (!message.guild.me.permissions.has("PERMISSIONS.FLAGS_MANAGE_ROLES")) return message.channel.send("**I Dont Have The Permissions T Add Roles! - [MANAGE_ROLES]**");
...
*/ 	
    ...
    collector.on('end', async (msgs, reason) => { 		if (reason == 'DONE') { 			
