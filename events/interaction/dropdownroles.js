    ...
    if(interaction.isSelectMenu()) {
        ...
        if(guild.dropdownRoles.length > 0) {
            if(interaction.customId === "dropdown_roles") {
                if(!interaction.guild.me.permissions.has("MANAGE_ROLES")) return interaction.reply({content: `${bot.crosss} • I am missing the required permissions to give you these roles!`}).catch(() => null)
                        ...
                        dd.roles.forEach((r) => {
                            if(r.role === interaction.values[0]) {
                                ...
                                if(hasrole) {
                                   ...
                                   .setDescription(`> ${bot.tick} • I have removed the <@&${r.role}> role from you!`)
                                   .setColor(bot.color)
                                   interaction.reply ({embeds: [embed], ephemeral: true}).catch(() => null)
                                } else {
                                    interaction.member.roles.add(r.role).catch(() => null)
                                    const embed = new MessageEmbed()
                                   .setDescription(`> ${bot.tick} • I have given you the <@&${r.role}> role!`)
                                   .setColor(bot.color)
                      interaction.reply ({embeds: [embed], ephemeral: true}).catch(() => null)
                                }
                            }
