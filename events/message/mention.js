...
/**
 * Handles responding to mentions of the bot's user ID.
 * Checks for a mention of the bot's ID in the message content.
 * If found, sends an embed with info about the bot and links to support server, invite, and dashboard.
 * Uses buttons for the links.
 */
	...
	try {
		...
		if (mentionRegex) {
			...
			const ping = new EmbedBuilder()
				.setDescription(
					` > ɛiɜ • **Hello I'm ${bot.user.username}**!\n\n > ↛ • You can see all my commands by running \`/helpp\`!\n > ↛ • I have a total of **${bot.guilds.cache.size}** servers, **${bot.users.cache.size}** users, ${bot.emojis.cache.size} emojis and ${totalCommands} commands !!`
				)
